...
module.exports=function(){
	...
	function ajaxUtil(){
		$.ajax({
			type: "GET",
			url:'localhost:8080/datajson/productdata.json',
	...
	function initArr(data){
		for(var i=0;i<data.length;i++){
			...
			if(data[i].Id.substring(0,2)=="31"){
				arrDS.push(strData);
			}else if(data[i].Id.substring(0,2)=="32"){
				arrPZ.push(strData);
			}else if(data[i].Id.substring(0,2)=="33"){
				arrDQ.push(strData);
			}else if(data[i].Id.substring(0,2)=="34"){
				arrYJ.push(strData);
			}else if(data[i].Id.substring(0,2)=="35"){
				arrSH.push(strData);
			}else if(data[i].Id.substring(0,2)=="36"){
				arrPJ.push(strData);
			}
		}
		showData(nowPage,type);
